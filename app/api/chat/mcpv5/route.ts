  ...
  try {
    ...
    const mappedMcpTools = mcpConfigsToUse
      .map(mcpServer => {
        ...
        // Logic to construct headers for toolDefinition.headers
        // This should be the robust logic from previous steps that handles api_key_env_var, auth_type, and fallbacks
        ...
        if (mcpServer.api_key_env_var) {
          ...
          if (!apiKey) {
        ...
        } else if (mcpServer.auth && mcpServer.auth.token) { 
            ...
            if (mcpServer.auth.type === 'bearer') {
                ...
                headers['X-API-Key'] = mcpServer.auth.token; 
            }
        }

        if (excludeTool) {
          return null; // This tool will be filtered out
        }
        
        if (Object.keys(headers).length > 0) {
          toolDefinition.headers = headers;
        }
