...
import { ArrowLeft, Send, RefreshCw, Loader2, X, Bot, User, Hammer, ChevronDown, ChevronUp, CheckCircle, Circle } from "lucide-react"; // Added CheckCircle, Circle
...
// Updated Tool type to include id
interface Tool {
  id: string; 
  name: string;
  description?: string;
...
export default function MCPv5AssistantChatPage() {
  const router = useRouter();
  const params = useParams();
  const assistantId = params.assistantId as string;

  const [assistantName, setAssistantName] = useState(`MCP v5 - ${assistantId}`);
  ...
  const [showToolSelector, setShowToolSelector] = useState(false); // Renamed from showTools
  ...
  const showWelcomeMessage = useCallback(() => {
    setMessages([{
      id: "welcome",
      role: "assistant",
      content: `¡Hola! Soy ${assistantName}. Tengo acceso a herramientas externas. ¿Cómo puedo ayudarte?`,
      timestamp: new Date(),
      isStreaming: false
    }]);
  }, [assistantName]);
  ...
  useEffect(() => {
    showWelcomeMessage();
    fetchAvailableTools(); // Fetch tools on mount
  ...
  useEffect(() => {
    // Update assistant name if assistantId changes (though typically it won't for a given page instance)
    setAssistantName(`MCP v5 - ${assistantId}`);
  }, [assistantId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const requestBody: { message: string; assistantId?: string; forced_tool_id?: string } = { 
        message: currentInput,
        assistantId: assistantId // Assuming your API might use this for specific MCP instances
      };
      if (selectedToolId) {
      ...
      if (data.success && data.response) {
        ...
        throw new Error(data.error || data.response || "Respuesta no exitosa");
      }
    ...
    <div className="flex flex-col min-h-screen text-white bg-gray-950">
      ...
      <header className="sticky top-0 z-20 flex items-center justify-between p-4 border-b border-white/10 bg-gray-900/60 backdrop-blur-md">
        ...
        <div className="flex flex-col items-center">
            <h1 className={`text-lg font-bold tracking-tight bg-clip-text text-transparent ${SubtleGradient}`}>
              {assistantName}
            </h1>
            {/* Informational tool count display below name, not interactive here, interactive part is in footer */}
        ...
        {/* Tool selector popover is moved to the footer section */}
      </header>
        ...
        <div className="max-w-3xl mx-auto">
            ...
            >
              ...
              {availableTools.length > 0 ? (
                <ul className="space-y-2 max-h-60 overflow-y-auto">
                  {availableTools.map((tool) => ( // Make sure 'tool' here has 'id'
                    <li key={tool.id}>
                      <button
          ...
          <form onSubmit={handleSubmit} className="flex items-end space-x-2 md:space-x-3">
            ...
            <textarea
              ...
              placeholder={`Mensaje a ${assistantName}...`}
          ...
          <div className="flex justify-center items-center mt-2.5">
            <p className="text-xs text-gray-500">
              {isLoading ? `${assistantName} está pensando...` : `Chat con ${assistantName}`}
            </p>
          </div>
        </div>
      </footer>
